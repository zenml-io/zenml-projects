...
class ConversationData(BaseModel):
    ...
    date_range: Dict[str, datetime] = Field(description="Start and end timestamps")
...
class RawConversationData(BaseModel):
    ...
    conversations: List[ConversationData] = Field(description="List of conversations")
...
class CleanedConversationData(BaseModel):
    ...
    conversations: List[ConversationData] = Field(description="Cleaned conversations")
    removed_messages_count: int = Field(description="Number of messages removed during cleaning")
...
class ProcessedData(BaseModel):
    ...
    run_id: str = Field(description="ZenML pipeline run identifier for trace tagging")
...
class DeliveryResult(BaseModel):
    ...
    delivered_items: List[str] = Field(description="Successfully delivered item IDs")
    failed_items: List[str] = Field(description="Failed delivery item IDs")
    delivery_url: Optional[str] = Field(default=None, description="URL of delivered content")
...
class EvaluationMetrics(BaseModel):
    ...
    task_extraction_accuracy: float = Field(description="Task extraction accuracy")
    ...
    human_feedback_score: Optional[float] = Field(default=None, description="Human feedback rating")
