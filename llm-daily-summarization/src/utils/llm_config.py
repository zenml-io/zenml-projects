...
def initialize_litellm_langfuse():
    ...
    if not all(
        ...
        logger.warning(
            "Langfuse credentials not found. LLM calls will not be traced."
        )
        return False
    ...
    try:
        ...
        logger.warning(
            "Continuing without Langfuse tracing. LLM calls will still work but won't be traced."
        )
...
def get_langfuse_project_id():
    ...
    if not project_id:
        logger.warning(
            "LANGFUSE_PROJECT_ID not set. Please set this environment variable to generate direct trace URLs."
        )
    return project_id
