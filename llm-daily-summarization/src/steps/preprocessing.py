...
class TextCleaner:
    ...
    def __init__(self):
        ...
        self.mention_pattern = re.compile(r'<@[!&]?[0-9]+>')  # Discord mentions
        self.slack_mention_pattern = re.compile(r'<@U[A-Z0-9]+>')  # Slack user mentions
        self.channel_mention_pattern = re.compile(r'<#[0-9]+>')  # Discord channel mentions
        self.emoji_pattern = re.compile(r':[a-zA-Z0-9_]+:')  # Custom emoji
        self.code_block_pattern = re.compile(r'```.*?```', re.DOTALL)  # Code blocks
    ...
    def is_system_message(self, message: ChatMessage) -> bool:
        ...
        special_char_ratio = sum(1 for c in message.content if not c.isalnum() and not c.isspace()) / max(len(message.content), 1)
...
def text_preprocessing_step(
    ...
    logger.info(f"Starting text preprocessing for {len(raw_data.conversations)} conversations")
    ...
    for conversation in raw_data.conversations:
        logger.info(f"Processing conversation: {conversation.channel_name} ({conversation.source})")
        ...
        for message in conversation.messages:
            if cleaner.should_keep_message(message):
                ...
                cleaned_content = cleaner.clean_message_content(message.content)
        ...
        if len(cleaned_messages) > max_messages_per_conversation:
            ...
            cleaned_messages = sorted(cleaned_messages, key=lambda x: x.timestamp)[-max_messages_per_conversation:]
            removed_count += len(conversation.messages) - max_messages_per_conversation
            processing_notes.append(f"Truncated {conversation.channel_name} to {max_messages_per_conversation} messages")
        ...
        if cleaned_messages:
            ...
            logger.info(f"Cleaned {conversation.channel_name}: {len(cleaned_messages)} messages kept, {removed_count} removed")
        else:
            processing_notes.append(f"Conversation {conversation.channel_name} removed - no valid messages")
            logger.warning(f"No valid messages remaining in {conversation.channel_name}")
    ...
    logger.info(f"Text preprocessing complete: {len(cleaned_conversations)} conversations, {total_word_count} words")
