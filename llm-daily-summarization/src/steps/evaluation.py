...
class PipelineEvaluator:
    ...
    def calculate_delivery_success_rate(self, delivery_results: List[DeliveryResult]) -> float:
    ...
    def calculate_processing_time(self, processed_data: ProcessedData) -> float:
        ...
        return processed_data.processing_metadata.get("processing_time_seconds", 0.0)
...
def evaluation_step(
    ...
    summary_quality_score = evaluator.evaluate_summary_quality(summaries_and_tasks)
    logger.info(f"Summary quality score: {summary_quality_score:.3f}")
    
    # Evaluate task extraction accuracy
    task_extraction_accuracy = evaluator.evaluate_task_extraction_accuracy(summaries_and_tasks)
    logger.info(f"Task extraction accuracy: {task_extraction_accuracy:.3f}")
    
    # Calculate delivery success rate
    delivery_success_rate = evaluator.calculate_delivery_success_rate(delivery_results)
