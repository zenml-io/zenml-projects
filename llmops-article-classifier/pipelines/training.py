...
def training_pipeline(config: Dict | None = None, save_to_disk: bool = False):
    ...
    if augmented_dataset_path and os.path.exists(augmented_dataset_path):
        ...
        raise ValueError("No valid dataset found. Check dataset paths in config.")
    ...
    training_params = config["steps"]["finetune_modernbert"]["parameters"].copy()

    base_model_id = config.get("parameters", {}).get("base_model_id") or config.get(
        "model_repo_ids", {}
    ).get("modernbert_base_model")
