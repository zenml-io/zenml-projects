...
def classify_articles(
    ...
    output_dir = os.path.join(f"classification_results/for_{classification_type}", timestamp)
    os.makedirs(output_dir, exist_ok=True)

    batch_articles = (
        articles[batch_config.batch_start : batch_config.batch_start + batch_config.batch_size]
        if batch_config
        else articles
    )

    remaining_articles = batch_articles
    if checkpoint_config and checkpoint_config.enabled and checkpoint_config.run_id:
        checkpoint_path = find_checkpoint_file(classification_type, checkpoint_config.run_id)
        if checkpoint_path:
            checkpoint_dir = os.path.dirname(checkpoint_path)
            checkpoint_data, completed_article_urls, checkpoint_progress = load_checkpoint_data(
                checkpoint_path
            )
    ...
    parallel_workers = parallel_config.workers if parallel_config and parallel_config.enabled else 1
    if parallel_workers < 1:
        logger.log_warning(f"Invalid parallel_workers value: {parallel_workers}, using 1 instead")
        parallel_workers = 1

    if parallel_config and parallel_config.enabled and len(remaining_articles) > 1:
    ...
    logger.log_checkpoint(f"Classification complete. Processed {len(filtered_results)} articles.")
