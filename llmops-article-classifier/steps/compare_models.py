...
from utils import ClaudeEvaluator, compute_classification_metrics, flatten_metrics
...
def compare_models(
    ...
    try:
        claude_results = loop.run_until_complete(evaluator.evaluate_batch(texts))
    finally:
        loop.close()

    # Filter valid results
    valid_claude_results = [r for r in claude_results if r.error is None]
    valid_indices = [i for i, r in enumerate(claude_results) if r.error is None]
    ...
    modernbert_performance = compute_classification_metrics((modernbert_logits, valid_labels))
    claude_performance = compute_classification_metrics((claude_logits, valid_labels))

    metrics = {
        "modernbert": {
            "performance": {k: float(v) for k, v in modernbert_performance.items()},
            "avg_latency": float(modernbert_latency),
            "cost_per_1000": float(modernbert_latency * 0.004 * (1000 / modernbert_batch_size)),
        },
        "claude": {
            "performance": {k: float(v) for k, v in claude_performance.items()},
            "avg_latency": float(np.mean([r.latency for r in valid_claude_results])),
            "tokens": {
                k: float(np.mean([getattr(r, f"{k}_tokens") for r in valid_claude_results]))
                for k in ["input", "output", "total"]
            },
            "error_rate": float(
                len([r for r in claude_results if r.error is not None]) / len(claude_results)
            ),
