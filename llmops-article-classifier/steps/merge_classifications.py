...
def merge_classifications(
    ...
    classified_records = transform_classification_results(classification_results)
    ...
    for record in classified_records:
        ...
        if identifier not in training_identifiers:
            if "text" in record:
                ...
                logger.log_warning(f"Skipping record without text: {record['meta']['url']}")
    ...
    logger.log_output_file(
        augmented_dataset_path, f"Augmented Dataset (added {len(records_to_add)} new articles)"
    )
