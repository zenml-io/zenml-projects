...
class BatchProcessingConfig(BaseModel):
    ...
    batch_size: int = Field(..., description="Number of articles to process in this batch", gt=0)
...
class ParallelProcessingConfig(BaseModel):
    ...
    enabled: bool = Field(True, description="Whether to use parallel processing")
...
class InferenceParamsConfig(BaseModel):
    ...
    max_sequence_length: int = Field(..., description="Maximum sequence length for the model")
    temperature: float = Field(..., description="Temperature for sampling", ge=0.0, le=1.0)
    top_p: float = Field(..., description="Top-p sampling parameter", ge=0.0, le=1.0)
    ...
    def validate_probability_params(cls, v):
        if not 0.0 <= v <= 1.0:
            raise ValueError("Probability parameters must be between 0.0 and 1.0")
        return v

    @model_validator(mode="after")
    def validate_token_lengths(self):
        if self.max_new_tokens >= self.max_sequence_length:
            raise ValueError("max_new_tokens must be less than max_sequence_length")
        return self
...
class CheckpointConfig(BaseModel):
    ...
    frequency: int = Field(
        default=10, description="Number of articles to process before saving a checkpoint", ge=1
    )

    run_id: Optional[str] = Field(
        None, description="Unique identifier for the run if resuming from a specific run"
    )
...
class ClassificationPipelineConfig(BaseModel):
    ...
    batch_processing: Optional[BatchProcessingConfig] = Field(
        None, description="Batch processing parameters, null to process entire dataset"
    )
    ...
    parallel_processing: ParallelProcessingConfig = Field(
        default_factory=ParallelProcessingConfig, description="Parallel processing configuration"
    )
    inference_params: InferenceParamsConfig = Field(
        default_factory=InferenceParamsConfig, description="Model inference parameters"
    )
...
class DatasetSourceConfig(BaseModel):
    ...
    source_type: Literal["artifact", "disk"] = Field(..., description="Source type for test data")
    path: Optional[str] = Field(
        None, description="Path to test set on disk (used if source_type is 'disk')"
    )
    artifact_name: Optional[str] = Field(
        None, description="Name of artifact (used if source_type is 'artifact')"
    )
    version: Optional[int] = Field(
        None, description="Version of artifact (used if source_type is 'artifact')"
    )
...
class BatchSizesConfig(BaseModel):
    ...
    modernbert: int = Field(..., description="Batch size for ModernBERT inference", gt=0)
    claude: int = Field(..., description="Batch size for Claude API calls", gt=0)


class ClaudeHaikuCostConfig(BaseModel):
    """Configuration for Claude Haiku cost calculations."""

    input_cost_per_1k: float = Field(..., description="Cost per 1K input tokens", ge=0.0)
    output_cost_per_1k: float = Field(..., description="Cost per 1K output tokens", ge=0.0)


class CostsConfig(BaseModel):
    """Configuration for cost calculations."""

    claude_haiku: ClaudeHaikuCostConfig = Field(..., description="Claude Haiku cost parameters")


class ModelComparisonPipelineConfig(BaseModel):
    """Configuration for the model comparison pipeline."""

    dataset: DatasetSourceConfig = Field(..., description="Test dataset configuration")
    batch_sizes: BatchSizesConfig = Field(..., description="Batch sizes for different models")
...
class DataSplitConfig(BaseModel):
    ...
    test_size: float = Field(
        ..., description="Proportion of data to use for testing", ge=0.0, le=1.0
    )
    validation_size: float = Field(
        ..., description="Proportion of non-test data to use for validation", ge=0.0, le=1.0
    )
...
class FinetuneConfig(BaseModel):
    ...
    parameters: TrainingConfig = Field(..., description="Model training parameters")
...
class StepsConfig(BaseModel):
    ...
    data_split: DataSplitConfig = Field(..., description="Dataset splitting configuration")
    finetune_modernbert: FinetuneConfig = Field(..., description="Model training parameters")
...
class OutputPathsConfig(BaseModel):
    ...
    ft_tokenizer: str = Field(..., description="Path to save finetuned tokenizer")
    classifications: str = Field(
        "classification_results", description="Path to save classification results"
    )
...
class ModelRepoIdsConfig(BaseModel):
    ...
    modernbert_base_model: str = Field(..., description="ModernBERT base model ID")
    huggingface_repo: str = Field(..., description="HuggingFace repository for deployment")
