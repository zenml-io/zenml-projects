...
def calculate_metrics(
            ...
            error_entries.append(
                {
                    "index": i,
                    "error": reason,
                    "url": classification.get("meta", {}).get("url", "unknown"),
                }
            )
    ...
    if not y_true or not y_pred:
        ...
        return {"error_count": total_errors, "error_types": dict(error_counts)}, error_entries
...
def log_metrics_to_console(
    ...
    # summary
    logger.log_process("\nAnalysis Summary:")
    logger.log_success(f"Valid samples: {total_samples} ({valid_samples_percentage:.1f}%)")
    logger.log_success(f"Error samples: {total_errors} ({100 - valid_samples_percentage:.1f}%)")
    ...
    # class distribution
    logger.log_process("\nClass Distribution:")
    logger.log_success(f"Positive class ratio: {class_distribution['positive_ratio']:.3f}")
    logger.log_success(f"Negative class ratio: {class_distribution['negative_ratio']:.3f}")
...
"""
    ...
    for pred_type, stats in conf_stats.items():
        markdown += (
            f"| {pred_type} | {stats['mean']:.3f} | {stats['std']:.3f} | {stats['count']} |\n"
        )
...
def calculate_and_save_metrics_from_json(
    ...
    if not os.path.exists(results_path):
        logger.log_warning(f"Results file not found at {results_path}. Metrics not calculated.")
        return
    ...
    try:
        ...
        logger.log_warning(f"Failed to update results file with metrics: {str(e)}")
