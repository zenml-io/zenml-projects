...
def classify_single_article(
    ...
    """
    max_tokens = inference_params["max_sequence_length"] - inference_params["max_new_tokens"]
...
def classify_article_safely(args: Tuple) -> Tuple[int, Dict]:
    ...
    try:
        result = classify_single_article(
            client=client, article=article, model_id=model_id, inference_params=inference_params
        )
        return index, result
...
def prepare_classification_json(
    ...
    for i, classification in enumerate(classifications):
        idx = str(i + 1)
        result_dict["results"]["is_accepted"][idx] = classification.get("is_accepted", False)
        result_dict["results"]["confidence"][idx] = classification.get("confidence", 0.0)
        result_dict["results"]["reason"][idx] = classification.get("reason", "")
        ...
        if "published_date" in meta:
            ...
            result_dict["results"]["meta_published_date"][idx] = str(meta.get("published_date", ""))
        if "title" in meta:
...
def process_articles_parallel(
    ...
    checkpoint_frequency = checkpoint_config.frequency if checkpoint_enabled else 0
    ...
    for i, article in enumerate(remaining_articles):
        original_idx = next(
            idx for idx, a in enumerate(batch_articles) if a.meta.url == article.meta.url
        )
        process_args.append((i, article, client, model_id, inference_params_dict))
        article_to_original_idx[i] = original_idx

    with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_workers) as executor:
        future_to_idx = {
            executor.submit(classify_article_safely, args): args[0] for args in process_args
        }
        ...
        for future in tqdm(
            ...
            try:
                ...
                original_idx = article_to_original_idx[temp_idx]
                logger.error(f"Article #{original_idx} generated an exception: {exc}")
            ...
            if checkpoint_enabled and progress % checkpoint_frequency == 0 and progress > 0:
                save_checkpoint(
                    results=results,
