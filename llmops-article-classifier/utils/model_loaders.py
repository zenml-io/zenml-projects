...
def load_base_model(
    ...
    if device == "cuda":
        if remote_execution:
            logger.info("Using optimized GPU configuration for remote execution")
            if (
                ...
                logger.info(f"Using mixed precision: {model_kwargs.get('torch_dtype')}")

    model = AutoModelForSequenceClassification.from_pretrained(base_model_id, **model_kwargs)
...
def load_tokenizer(base_model_id: str, use_fast: bool = True) -> PreTrainedTokenizerFast:
