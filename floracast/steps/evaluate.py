...
@step
def evaluate(
    model: Annotated[object, "trained_model"], 
    train_series: Annotated[TimeSeries, "train_series"], 
    val_series: Annotated[TimeSeries, "val_series"],
    horizon: int = 7,
    metric: str = "smape"
    ...
    try:
        ...
        if hasattr(model, 'predict'):
            predictions = model.predict(n=min(horizon, len(val_series)), series=train_series)
