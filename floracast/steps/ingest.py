...
def generate_ecommerce_data(num_days: int = 180) -> pd.DataFrame:
    ...
    weekly_pattern = np.array([0.8, 0.85, 0.9, 0.95, 1.0, 1.3, 1.2])  # Mon to Sun
...
def ingest_data(
    ...
    if data_source == 'ecommerce_default':
        ...
        if not os.path.exists(csv_file_path):
            logger.info("Default ecommerce data not found, generating new data")
            os.makedirs('data', exist_ok=True)
            df = generate_ecommerce_data()
            df.to_csv(csv_file_path, index=False)
            logger.info(f"Generated and saved ecommerce data to {csv_file_path}")
        else:
            logger.info(f"Loading existing ecommerce data from {csv_file_path}")
    ...
    logger.info(f"Target stats: mean={df[target_col].mean():.2f}, std={df[target_col].std():.2f}")
