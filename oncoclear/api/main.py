...
MODEL_ARTIFACT_NAME = os.getenv("MODEL_ARTIFACT_NAME", "sklearn_classifier") # Updated default artifact name
PREPROCESS_PIPELINE_NAME = os.getenv("PREPROCESS_PIPELINE_NAME", "preprocess_pipeline") # Added preprocessing pipeline artifact name
...
async def lifespan(app: FastAPI):
    ...
    if zenml_key != "Not set":
        masked_key = zenml_key[:15] + "..." + zenml_key[-10:] if len(zenml_key) > 30 else "***masked***"
    else:
        masked_key = "Not set"
    ...
    logger.info(f"ZENML_CONFIG_DIR: {os.getenv('ZENML_CONFIG_DIR', 'Not set')}")
    
    try:
        logger.info(f"Attempting to load model '{MODEL_NAME}' version '{MODEL_STAGE}'.")
        ...
        logger.info(f"Found model version {model_version.name}. Loading model artifact '{artifact_name}'...")
        app_state["model"] = model_version.get_artifact(artifact_name).load()
        logger.info(f"Successfully loaded model artifact '{artifact_name}' from {MODEL_NAME}:{model_version.name}")
        
        # Also load the preprocessing pipeline artifact if available
        preprocess_name = PREPROCESS_PIPELINE_NAME
        try:
            logger.info(f"Attempting to load preprocessing pipeline artifact '{preprocess_name}'...")
            app_state["preprocess_pipeline"] = model_version.get_artifact(preprocess_name).load()
            logger.info(f"Successfully loaded preprocessing pipeline artifact.")
        except Exception as e:
            logger.warning(f"Failed to load preprocessing pipeline: {e}. Predictions may require pre-processed input.")
...
async def predict(payload: FeaturesPayload):
    ...
    try:
        ...
        prediction_value = prediction_result[0] if isinstance(prediction_result, (list, tuple, np.ndarray)) and len(prediction_result) > 0 else prediction_result
        ...
        try:
            ...
            logger.debug(f"Logging prediction metadata to model {MODEL_NAME}:{MODEL_STAGE}")
            log_metadata(
