...
@step
def fetch_repos(username: str, access_token: str, include_fork: bool = False) -> Output(repos=list):
...
if __name__ == "__main__":
    # Define the steps
    fetch_repos_step = fetch_repos(username=ORG, access_token=os.environ["GH_ACCESS_TOKEN"])
    clone_repos_step = clone_repos(mirror_directory=MIRROR_DIRECTORY)
    read_and_filter_files_step = read_and_filter_files(mirror_directory=MIRROR_DIRECTORY)
    upload_to_hf_hub_step = upload_to_hf_hub(dataset_id=DATASET_ID, file_format=FEATHER_FORMAT)
