...
def load_data(
    ...
    Annotated[Path, "raw_train_data_path"],
    Annotated[Path, "raw_test_data_path"],
]:
    ...
    Returns:
        Tuple with "raw_train_data_path" and "raw_test_data_path" paths on PVC.
    """
    root_dir = Path(mount_path) / data_version
    raw_train_data_dir = root_dir / "train"
    raw_test_data_dir = root_dir / "test"

    # Cache: if raw train already has .npy files, skip download
    if raw_train_data_dir.exists() and raw_test_data_dir.exists():
        npy_count = sum(1 for _ in raw_train_data_dir.rglob("*.npy")) + sum(
            1 for _ in raw_test_data_dir.rglob("*.npy")
        )
        if npy_count > 0:
            print(
                f"Using cached raw data for {data_version=} ({npy_count} .npy files)"
            )
            return raw_train_data_dir, raw_test_data_dir
    ...
    for name, dest in (
        ("train", raw_train_data_dir),
        ("test", raw_test_data_dir),
    ):
