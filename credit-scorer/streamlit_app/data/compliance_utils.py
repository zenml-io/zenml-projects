...
from src.utils.compliance.compliance_constants import (
    EU_AI_ACT_ARTICLES,
...
def get_compliance_results(
    ...
    try:
        ...
        if not valid:
            st.warning(f"Release directory missing required files: {', '.join(missing_files)}")
...
def format_compliance_findings(findings: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
...
def get_compliance_data_sources(results: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if release_dir.exists():
        ...
        for filename, display_name in [
            ...
            if file_path.exists():
                ...
                timestamps[display_name] = pd.Timestamp(timestamp, unit='s').strftime("%Y-%m-%d %H:%M")
    ...
    if "articles" in results:
        ...
        for article_id, data in article_data.items():
            ...
            if score > summary["strongest_article"]["score"]:
                summary["strongest_article"] = {"name": display_name, "score": score}
            
            if score < summary["weakest_article"]["score"]:
                summary["weakest_article"] = {"name": display_name, "score": score}
