...
def validate_yaml_file(filepath: Union[str, Path]) -> Tuple[bool, Optional[Dict[str, Any]], List[str]]:
...
def validate_json_file(filepath: Union[str, Path]) -> Tuple[bool, Optional[Dict[str, Any]], List[str]]:
...
def validate_risk_register(filepath: Union[str, Path] = RISK_REGISTER_PATH) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if missing_columns:
            errors.append(f"Required columns missing from risk register: {', '.join(missing_columns)}")
...
def validate_release_directory(release_id: str) -> Tuple[bool, Dict[str, Dict[str, Any]]]:
    ...
    if not release_dir.exists() or not release_dir.is_dir():
        return False, {"directory": {"exists": False, "errors": ["Release directory not found"]}}
    ...
    required_files = {
        ...
        "annex_iv_cs_deployment.md": lambda f: (os.path.exists(f), None, [] if os.path.exists(f) else ["File not found"]),
    }
...
def show_validation_summary(validation_results: Dict[str, Dict[str, Any]]) -> None:
    ...
    """
    # Count issues
    missing_files = [name for name, result in validation_results.items() 
                    if name != "directory" and not result.get("exists", False)]
    
    invalid_files = [name for name, result in validation_results.items() 
                    if name != "directory" and result.get("exists", False) 
