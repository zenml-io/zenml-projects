...
def transform_classification_results(results: Dict) -> List[Dict]:
    ...
    valid_indices = [
        ...
        if "Validation Error"
        not in results["results"].get("reason", {}).get(idx, "")
    ]
    ...
    for idx in valid_indices:
        if not all(
            field.get(idx)
            for field in [
                results["results"]["meta_url"],
                results["results"]["meta_title"],
            ]
        ):
            continue
        ...
        record = {
            "meta": {
                ...
                "published_date": results["results"][
                    "meta_published_date"
                ].get(idx, ""),
                "author": results["results"]["meta_author"].get(idx, ""),
            },
            "answer": "accept"
            if results["results"]["is_accepted"][idx]
            else "reject",
        }
