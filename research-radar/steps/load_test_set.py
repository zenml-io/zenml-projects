...
def load_test_set(
    ...
    Dataset,
    ArtifactConfig(
        name="test_set_artifact", run_metadata={"source": "training_pipeline"}
    ),
]:
    ...
    if source_type not in ["disk", "artifact"]:
        raise ValueError(
            f"Invalid source_type: {source_type}. Must be 'disk' or 'artifact'"
        )
    ...
    try:
        if source_type == "disk":
            if not path:
                raise ValueError(
                    "Path must be provided when loading from disk"
                )
            ...
            if not artifact_name:
                raise ValueError(
                    "Artifact name must be provided when loading from artifact"
                )
            client = Client()
