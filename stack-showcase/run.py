...
@click.option(
    "--custom-training-suffix",
    default="",
    type=click.STRING,
    help="Suffix to append to the training pipeline name.",
)
@click.option(
    "--no-cache",
    is_flag=True,
...
def main(
    ...
    Args:
        ...
        custom_training_suffix: The suffix to append to the pipeline configuration file when training.
    ...
    if training_pipeline:
        ...
        if no_cache:
            pipeline_args["enable_cache"] = False
        pipeline_args["config_path"] = os.path.join(
            config_folder, f"training_sgd{custom_training_suffix}.yaml"
        )
