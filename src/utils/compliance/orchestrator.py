...
from .calculators.config_driven_calculator import ConfigDrivenCalculator
from .compliance_constants import DEFAULT_COMPLIANCE_PATHS, EU_AI_ACT_ARTICLES
    ...
    def calculate_full_compliance(
        ...
        try:
            ...
            for article_id in articles_to_process:
                try:
                    calculator = ConfigDrivenCalculator(self.config_path, article_id)
    ...
    def _load_common_data(
        ...
        if release_id:
            risk_scores_path = os.path.join(
                DEFAULT_COMPLIANCE_PATHS["releases_dir"], release_id, "risk_scores.yaml"
            )
