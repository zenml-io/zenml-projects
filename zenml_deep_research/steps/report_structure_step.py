...
from utils.helper_functions import remove_reasoning_from_output, clean_json_tags, safe_json_loads
...
def report_structure_step(
    ...
    try:
        ...
        if not report_structure:
            logger.warning("Failed to parse report structure, using fallback structure")
            report_structure = [
                {"title": "Introduction", "content": "Introduction to the topic"},
                {"title": "Main Findings", "content": "Key findings about the topic"},
                {"title": "Conclusion", "content": "Concluding thoughts and summary"}
            ]
        ...
        logger.info(f"Generated report structure with {len(state.paragraphs)} paragraphs")
        ...
        return State(
            query=query,
            paragraphs=[
                Paragraph(title="Introduction", content="Introduction to the topic"),
                Paragraph(title="Main Findings", content="Key findings about the topic"),
                Paragraph(title="Conclusion", content="Concluding thoughts and summary")
            ]
        ) 