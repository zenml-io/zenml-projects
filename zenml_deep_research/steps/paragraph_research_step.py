...
def _get_reflection_search_query(
    ...
    try:
        ...
        if not result or "search_query" not in result:
            # Fallback if parsing fails
            return {
                "search_query": f"additional information about {title}",
                "reasoning": "",
            }
...
def _update_paragraph_with_reflection(
    ...
    try:
        ...
        return result.get(
            "updated_paragraph_latest_state", paragraph_latest_state
        )
...
def paragraph_research_step(
    ...
    """
    logger.info(
        f"Starting research for {len(current_state.paragraphs)} paragraphs"
    )
    ...
    for i, paragraph in enumerate(researched_state.paragraphs):
        logger.info(
            f"Researching paragraph {i + 1}/{len(researched_state.paragraphs)}: {paragraph.title}"
        )
        ...
        search_query = search_query_data.get(
            "search_query", f"research about {paragraph.title}"
        )
        ...
        for j in range(num_reflections):
            logger.info(
                f"Performing reflection {j + 1}/{num_reflections} for paragraph {i + 1}"
            )
            ...
            reflection_query = reflection_query_data.get(
                "search_query", f"additional info about {paragraph.title}"
            )

            # Step 5b: Perform reflection search
            logger.info(
                f"Performing reflection search with query: {reflection_query}"
            )
            ...
            paragraph.research.latest_summary = (
                _update_paragraph_with_reflection(
                    title=paragraph.title,
                    content=paragraph.content,
                    search_query=reflection_results.get(
                        "query", reflection_query
                    ),
                    search_results=raw_contents,
                    paragraph_latest_state=paragraph.research.latest_summary,
