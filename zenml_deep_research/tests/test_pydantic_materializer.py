...
def test_materializer_save_and_load(sample_state: ResearchState):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        assert loaded_state.get_current_stage() == sample_state.get_current_stage()
        
        # Check that key fields were preserved
        question = "Economic impacts"
        assert loaded_state.synthesized_info[question].synthesized_answer == \
            sample_state.synthesized_info[question].synthesized_answer
        assert loaded_state.synthesized_info[question].confidence_level == \
...
def test_html_generation_stages(sample_state: ResearchState):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        assert "Synthesized Information" in html  # Should show synthesized info
        ...
        viewpoint_analysis = ViewpointAnalysis(
            ...
            areas_of_tension=[
                ViewpointTension(
                    topic="Job impacts",
                    viewpoints={"Positive": "New green jobs", "Negative": "Job losses"}
                )
            ]
        )
