...
from utils.tracing_metadata_utils import identify_prompt_type
...
def test_identify_prompt_type():
    ...
    test_cases = [
        (
            "You are a Deep Research assistant specializing in effective search query generation.",
            "search_query",
        ),
        (
            "Given the MAIN RESEARCH QUERY and DIFFERENT DIMENSIONS sub-questions",
            "query_decomposition",
        ),
        (
            "Your task is information synthesis with comprehensive answer and confidence level",
            "synthesis",
        ),
    ...
    for content, expected in test_cases:
        ...
        print(
            f"  {status} Content: '{content[:50]}...' => {result} (expected: {expected})"
        )
...
def test_tracing_metadata_with_prompts():
    ...
    print(
        f"  ✓ Created TracingMetadata with {len(metadata.prompt_metrics)} prompt types"
    )
    print(f"    Total pipeline cost: ${metadata.total_cost:.4f}")
    print("\n  Prompt breakdown:")
    for metric in metadata.prompt_metrics:
        print(
            f"    - {metric.prompt_type}: ${metric.total_cost:.4f} ({metric.percentage_of_total_cost:.1f}%)"
        )
