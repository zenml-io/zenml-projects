    ...
    for idx, row in df.iterrows():
        ...
        completion = client.beta.chat.completions.parse(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Extract logical sections from financial reports.",
                },
...
def llm_extract_metadata_step(df: pd.DataFrame) -> Dict[str, DocumentMetadata]:
    ...
    for idx, row in df.iterrows():
        metrics = {
            ...
            "total_identifiable_intangible_assets": row.get(
                "total_identifiable_intangible_assets", None
            ),
            ...
            "total_stockholders_equity": row.get(
...
@step
def store_structured_data_step(
    sections_data: Dict[str, DocumentSections], metadata_data: Dict[Any, Any]
) -> Annotated[
    Dict[str, Any],
    ArtifactConfig(name="structured_dataset", tags=["financial", "FindSum"]),
]:
    """
