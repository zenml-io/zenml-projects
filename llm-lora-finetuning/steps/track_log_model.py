...
def track_log_model(
    ...
    Args:
        ...
        trainer_args: The trainer arguments to log.
        use_fast: Whether to use the fast tokenizer.
        is_eval: Whether to evaluate the model.
    ...
    signature = infer_signature(
        model_input=sample["target"],
        model_output=sample["meaning_representation"],
        # Parameters are saved with default values if specified
        params={"max_new_tokens": 100, "pad_token_id": 2},
    )
    
    # If you interrupt the training, uncomment the following line to stop the MLflow run
    mlflow.log_params(trainer_args)
